...
const MyAccount = () => {
  ...
  const updateUserInfo = async () => {
    await toast.promise(
      axiosInstance.post("/api/user/updateuser", { name: userInfo.name, address: userInfo.address, pincode: userInfo.pincode, phone: userInfo.phone }),
      {
        pending: "Please wait...",
        success: {
          render({ data }) {
  ...
  const updateUserPassword = async () => {
    if (userInfo.newPassword && userInfo.confirmPassword && userInfo.newPassword === userInfo.confirmPassword) {
      await toast.promise(
        axiosInstance.post("/api/user/updateuserpassword", { newPassword: userInfo.newPassword, currentPassword: userInfo.currentPassword }),
        {
          pending: "Please wait...",
          success: {
            render({ data }) {
