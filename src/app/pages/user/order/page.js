...
const MyOrder = async ({ searchParams }) => {
  ...
  return (
    <section className="text-gray-600 body-font overflow-hidden">
      <div className="px-5 py-8 lg:py-28">
            ...
            </div>
            {
              order?.products?.map((product, i) => (
                <div key={i} className="flex border-t border-gray-200 py-2">
                  ...
                  <span className="ml-auto mr-auto text-gray-900">₹{product?.price * product?.qty}</span>
                </div>
              ))
            }
            <div className="flex justify-between items-center my-6">
              <span className="title-font font-medium text-[22px] text-gray-900">SubTotal: ₹{order?.checkoutAmount}</span>
              <div>
